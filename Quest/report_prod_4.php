...
/**
 * 
 * TABELA DE ÍNDICES E CATEGORIAS
 * 
 * */
...
$title = 'Tabela dos Índices e Categorias nos Modelos de Crenças Determinantes';
...
if (!$pesquisa->isProdutoAdquirido(4)) {
    echo convertIsoUtf("Acesso negado a este relatório.");
    return;
}
...
<td width='80'>Núm. do Sujeito</td>";
	...
	foreach ($quests as $quest) {
		if ($quest->infos['StatusId'] == 3) {
			...
			foreach ($quest->fatores as $fator) $table .= "<td width='".$pdf->getCellWidthByFatorId($fator->id)."' ALIGN='CENTER'>$fator->classificacao</td>";
			$table .= "</tr>";
		}
	}
	$table .= "</table>
...
$pdf->Output($pesquisa->id . ' - ' . convertIsoUtf($pesquisa->titulo) . 
                convertIsoUtf(' - Tabela_Categorias-Índices_Resiliência.pdf'), 'D');
?>