...
$title = 'Tabela das Categorias nos Modelos de Crenças Determinantes';
$pdf->SetTitle($title);
...
if (!$pesquisa->isProdutoAdquirido(5)) {
    echo utf8_decode("Acesso negado a este relatório.");
    return;
}
//Logo
...
//Print table
$pdf->ChapterTitle('Tabela das categorias', true, true, true);
//$quests = $pesquisas->QuestListByPesquisaId($pesquisaid);
$quests = $pesquisa->getQuestionariosByStatus(QUESTIONARIO_STATUS_CONCLUIDO);

if ($quests) {
    $pdf->multiCellTableWidths = array(80, 120, 120, 120, 120, 120, 120, 120, 120);
    $pdf->multiCellTableAligns = array('L', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C');
    
    //table header
    $pdf->SetFont('Arial', 'B', 7);
    $data[] = utf8_decode('Núm. do Sujeito');
    foreach ($quests[0]->fatores as $fator) 
		$data[] = utf8_decode($fator->nome);
	
    $pdf->MutiCellTableRow($data);
    ...
    foreach ($quests as $quest) {
		if ($quest->infos['StatusId'] == 3) {
			...
			foreach ($quest->fatores as $fator) 
                $data[] = utf8_decode($fator->classificacao);
...
$pdf->Output('Tabela_Categorias.pdf', 'D');
?>