...
require_once ('../App_Code/FileHandler.class');
...
$pdf->SetTextColor(0);
$pdf->Cell(85, 9, utf8_decode('Tabela: Condições de Fraca Resiliência na equipe no PC - P'), 0, 10);
...
$pdf->SetFont('Verdana', 'B', 10);
$pdf->Cell(55, 8, utf8_decode('MCD'), 0, 0, 'C', true);
$pdf->Cell(15, 8, utf8_decode('Qtde'), 0, 0, 'C', true);
$pdf->Cell(105, 8, utf8_decode('Comentários'), 0, 0, 'C', true);
$pdf->Ln(8);
...
foreach ($items as $fator) {	
	...
	$pdf->Cell(55, 8, utf8_decode($fator->nome), 0, 0, 'L', true);
	
	$pdf->SetFillColor(240);
	$pdf->SetTextColor(255,0,0);
	$pdf->SetFont('Verdana', '', 14);
	$pdf->Cell(15, 8, utf8_decode($fator->qtde), 0, 0, 'C', true);	
	
	$pdf->SetFillColor(250); 
  	$pdf->SetTextColor(99);
	$pdf->SetFont('Verdana', '', 8);
	if (strlen(utf8_decode($fator->descricaoFracaResilienciaPCI)) > 70) $line_height = 4; else $line_height = 8;
	$pdf->MultiCell(105, $line_height, utf8_decode($fator->descricaoFracaResilienciaPCI), 0, 'L', true);
...
$pdf->SetFont('Verdana', 'B', 10);
$pdf->SetTextColor(0);
$pdf->Cell(85, 9, utf8_decode('Gráfico: Categoria FRACA no estilo PC - P'), 0, 1);
...
$pdf->SetFont('Verdana', 'B', 10);
$pdf->SetTextColor(0);
$pdf->Cell(85, 9, utf8_decode('Tabela: Condições de Fraca Resiliência na equipe no PC - I'), 0, 1);
...
foreach ($items as $fator) {	
	...
	$pdf->SetFillColor(250); 
  	$pdf->SetTextColor(99);
	$pdf->SetFont('Verdana', '', 8);
	if (strlen(utf8_decode($fator->descricaoFracaResilienciaPCP)) > 70) $line_height = 4; else $line_height = 8;
	$pdf->MultiCell(105, $line_height, utf8_decode($fator->descricaoFracaResilienciaPCP), 0, 'L', true);
...
$pdf->SetFont('Verdana', 'B', 10);
$pdf->SetTextColor(0);
$pdf->Cell(85, 9, utf8_decode('Gráfico: Categoria FRACA no estilo PC - I'), 0, 1);
